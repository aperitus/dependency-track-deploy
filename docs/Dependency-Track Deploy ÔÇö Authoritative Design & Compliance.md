...
| `DTRACK_ALPINE_DATABASE_DRIVER` | Var | No | `org.postgresql.Driver` | Optional override. When `ALPINE_DATABASE_MODE=external`, the workflow renders this to the app config secret as `ALPINE_DATABASE_DRIVER`. Required for Azure Postgres Flexible Server with some JVM/JDBC builds. |
